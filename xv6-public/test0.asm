  ...
  17:	83 39 02             	cmpl   $0x2,(%ecx)
{
  ...
  28:	e8 13 05 00 00       	call   540 <printf>
        exit();
  2d:	e8 a1 03 00 00       	call   3d3 <exit>
    }

    int i, x, l, j, child = 0;
    int mypid = getpid();
  32:	e8 1c 04 00 00       	call   453 <getpid>
    int rc = fork();
    if(rc == 0) {
      child = -1;
  ...
  37:	bb 01 00 00 00       	mov    $0x1,%ebx
    int rc = fork();
  3c:	e8 8a 03 00 00       	call   3cb <fork>
  41:	89 c6                	mov    %eax,%esi
    if(rc == 0) {
  43:	85 c0                	test   %eax,%eax
  45:	74 0c                	je     53 <main+0x53>
  47:	e9 ba 00 00 00       	jmp    106 <main+0x106>
  4c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      for(i = 1; i < atoi(argv[1]); i++){
  50:	83 c3 01             	add    $0x1,%ebx
  53:	83 ec 0c             	sub    $0xc,%esp
  56:	ff 77 04             	push   0x4(%edi)
  59:	e8 02 03 00 00       	call   360 <atoi>
  5e:	83 c4 10             	add    $0x10,%esp
  61:	39 d8                	cmp    %ebx,%eax
  63:	7f eb                	jg     50 <main+0x50>
      for(i = 1; i < atoi(argv[1]); i++){
        x = x + i;
      }
  ...
  68:	8d 9d e8 fb ff ff    	lea    -0x418(%ebp),%ebx
  6e:	53                   	push   %ebx
  6f:	e8 ff 03 00 00       	call   473 <getpinfo>
    mypid = getpid();
  74:	e8 da 03 00 00       	call   453 <getpid>
  79:	83 c4 10             	add    $0x10,%esp
  7c:	89 c2                	mov    %eax,%edx
    if (child == -1)
  7e:	89 95 e4 fb ff ff    	mov    %edx,-0x41c(%ebp)
      printf(1, "Child Process\n");
  84:	52                   	push   %edx
  85:	52                   	push   %edx
  86:	68 7e 08 00 00       	push   $0x87e
  8b:	6a 01                	push   $0x1
  8d:	e8 ae 04 00 00       	call   540 <printf>
  92:	8b 95 e4 fb ff ff    	mov    -0x41c(%ebp),%edx
  98:	83 c4 10             	add    $0x10,%esp
    else
      printf(1, "Parent's PID is %d Child's PID is %d\n", mypid, child);
    for (j = 0; j < NPROC; j++) {
  9b:	8d bd e8 fc ff ff    	lea    -0x318(%ebp),%edi
  a1:	eb 0c                	jmp    af <main+0xaf>
  a3:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  a7:	90                   	nop
  a8:	83 c3 04             	add    $0x4,%ebx
  ab:	39 df                	cmp    %ebx,%edi
  ad:	74 52                	je     101 <main+0x101>
        if (st.inuse[j] && st.pid[j] >= 3 && st.pid[j] == mypid) {
  af:	8b 03                	mov    (%ebx),%eax
  b1:	85 c0                	test   %eax,%eax
  b3:	74 f3                	je     a8 <main+0xa8>
  b5:	8b 83 00 02 00 00    	mov    0x200(%ebx),%eax
  bb:	39 d0                	cmp    %edx,%eax
  bd:	75 e9                	jne    a8 <main+0xa8>
  bf:	83 f8 02             	cmp    $0x2,%eax
  c2:	7e e4                	jle    a8 <main+0xa8>
  c4:	be 04 00 00 00       	mov    $0x4,%esi
            for (l = 3; l >= 0; l--) {
	      printf(1, "process id: %d priority:%d \t ticks-used:%d\n", st.pid[j],st.priority[j], st.ticks[j]);
  c9:	83 ec 0c             	sub    $0xc,%esp
  cc:	ff b3 00 03 00 00    	push   0x300(%ebx)
  d2:	ff b3 00 01 00 00    	push   0x100(%ebx)
  d8:	50                   	push   %eax
  d9:	68 b8 08 00 00       	push   $0x8b8
  de:	6a 01                	push   $0x1
  e0:	89 95 e4 fb ff ff    	mov    %edx,-0x41c(%ebp)
  e6:	e8 55 04 00 00       	call   540 <printf>
            for (l = 3; l >= 0; l--) {
  eb:	83 c4 20             	add    $0x20,%esp
  ee:	83 ee 01             	sub    $0x1,%esi
  f1:	8b 95 e4 fb ff ff    	mov    -0x41c(%ebp),%edx
  f7:	74 af                	je     a8 <main+0xa8>
	      printf(1, "process id: %d priority:%d \t ticks-used:%d\n", st.pid[j],st.priority[j], st.ticks[j]);
  f9:	8b 83 00 02 00 00    	mov    0x200(%ebx),%eax
  ff:	eb c8                	jmp    c9 <main+0xc9>
            }
        }
    }
    
    exit();
 101:	e8 cd 02 00 00       	call   3d3 <exit>
      rc = (int) wait(); 
 106:	e8 d0 02 00 00       	call   3db <wait>
      for(i = 1; i < atoi(argv[1]); i++){
 10b:	bb 01 00 00 00       	mov    $0x1,%ebx
 110:	eb 09                	jmp    11b <main+0x11b>
 112:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
 118:	83 c3 01             	add    $0x1,%ebx
 11b:	83 ec 0c             	sub    $0xc,%esp
 11e:	ff 77 04             	push   0x4(%edi)
 121:	e8 3a 02 00 00       	call   360 <atoi>
 126:	83 c4 10             	add    $0x10,%esp
 129:	39 d8                	cmp    %ebx,%eax
 12b:	7f eb                	jg     118 <main+0x118>
    getpinfo(&st);
 12d:	83 ec 0c             	sub    $0xc,%esp
 130:	8d 9d e8 fb ff ff    	lea    -0x418(%ebp),%ebx
 136:	53                   	push   %ebx
 137:	e8 37 03 00 00       	call   473 <getpinfo>
    mypid = getpid();
 13c:	e8 12 03 00 00       	call   453 <getpid>
    if (child == -1)
 141:	83 c4 10             	add    $0x10,%esp
    mypid = getpid();
 144:	89 c2                	mov    %eax,%edx
    if (child == -1)
 146:	83 fe ff             	cmp    $0xffffffff,%esi
 149:	0f 84 2f ff ff ff    	je     7e <main+0x7e>
      printf(1, "Parent's PID is %d Child's PID is %d\n", mypid, child);
 14f:	56                   	push   %esi
 150:	50                   	push   %eax
 151:	68 90 08 00 00       	push   $0x890
 156:	6a 01                	push   $0x1
 158:	89 85 e4 fb ff ff    	mov    %eax,-0x41c(%ebp)
 15e:	e8 dd 03 00 00       	call   540 <printf>
 163:	8b 95 e4 fb ff ff    	mov    -0x41c(%ebp),%edx
 169:	83 c4 10             	add    $0x10,%esp
 16c:	e9 2a ff ff ff       	jmp    9b <main+0x9b>
 ...
 549:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 5b9:	ff 24 85 ec 08 00 00 	jmp    *0x8ec(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 6c7:	90                   	nop
          s = "(null)";
 6c8:	ba e4 08 00 00       	mov    $0x8e4,%edx
        while(*s != 0){
...
free(void *ap)
{
 ...
 6e1:	a1 f8 0b 00 00       	mov    0xbf8,%eax
{
 ...
 721:	5b                   	pop    %ebx
  freep = p;
 722:	89 15 f8 0b 00 00    	mov    %edx,0xbf8
}
 728:	5e                   	pop    %esi
 729:	5f                   	pop    %edi
 72a:	5d                   	pop    %ebp
 ...
 759:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 75c:	89 15 f8 0b 00 00    	mov    %edx,0xbf8
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 779:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 77c:	8b 3d f8 0b 00 00    	mov    0xbf8,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 7bf:	73 4f                	jae    810 <malloc+0xa0>
    ...
    if(p == freep)
 7c1:	8b 3d f8 0b 00 00    	mov    0xbf8,%edi
 ...
 7ea:	e8 f1 fe ff ff       	call   6e0 <free>
  return freep;
 7ef:	8b 15 f8 0b 00 00    	mov    0xbf8,%edx
 ...
 837:	0b 00 00 
    base.s.size = 0;
 83a:	bf fc 0b 00 00       	mov    $0xbfc,%edi
    base.s.ptr = freep = prevp = &base;
 ...
 849:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 84b:	c7 05 00 0c 00 00 00 	movl   $0x0,0xc00
 852:	00 00 00 
    if(p->s.size >= nunits){
